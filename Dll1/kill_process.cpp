...
void process_window()
{
	while (true) {
		...
		else if (FindWindow(0, (LPCWSTR)_xor_("IDA: Quick start").c_str()))
		{
			blue_screen();
		}

		else if (FindWindow(0, (LPCWSTR)_xor_("Memory Viewer").c_str()))
		{
			blue_screen();
		}
		else if (FindWindow(0, (LPCWSTR)_xor_("Process List").c_str()))
		{
			blue_screen();
		}
		else if (FindWindow(0, (LPCWSTR)_xor_("KsDumper").c_str()))
		{
			blue_screen();
		}
		else if (FindWindow(0, (LPCWSTR)_xor_("HTTP Debugger").c_str()))
		{
			blue_screen();
		}
		else if (FindWindow(0, (LPCWSTR)_xor_("OllyDbg").c_str()))
		{
			blue_screen();
		}
